...
### CMS 後台管理
- **產品管理**：新增、編輯、刪除產品，設定分類、價格、尺寸、加料選項、**顯示價格開關**（可逐一控制前台是否顯示該產品價格），Slug 自動隨機生成，支援圖片上傳至 R2
- **分類管理**：管理產品分類
...
- **聯絡訊息**：查看和管理客戶聯絡訊息
- **網站設定**：設定網站基本資訊
- **自動登出**：Token 時效 1 小時，過期時自動導向登入頁面（每 60 秒檢查 session 狀態）
- **登入頁保護**：已登入用戶訪問登入頁時自動導向 Dashboard（防止瀏覽器上一頁造成的混淆）
...
| 設定位置 | 管理內容 |
|---------|---------|
| **網站設定** | 網站名稱、客服電話、電子郵件、地址、營業時間、社群連結、版權聲明 |
| **頁面內容** | 品牌描述、備用電話、加盟專用信箱、休息日說明等額外資訊 |

「網站設定」的資料會自動套用到前台的 Footer、Contact、Navigation、Franchise 等組件。
...
| Contact（額外資訊） | 備用電話、加盟專用信箱、休息日說明 |
| Footer（額外資訊） | 品牌描述 |
