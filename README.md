...
### CMS 後台管理
- **產品管理**：新增、編輯、刪除產品，設定分類、價格、尺寸、加料選項（Slug 自動隨機生成），支援圖片上傳至 R2
- **分類管理**：管理產品分類
- **門市管理**：管理門市據點資訊（地址、營業時間、地圖連結、線上訂購連結），支援圖片上傳至 R2
- **活動管理**：管理活動與促銷，支援圖片上傳至 R2
- **加盟方案**：管理加盟方案（投資金額、方案特色、包含項目），支援圖片上傳至 R2
...
- 門市的線上訂購連結（orderUrl）
- 活動圖片上傳
- 社群連結（可在網站設定或頁面內容的 footer 區塊編輯）

### 網站設定 vs 頁面內容

後台有兩個地方可以設定聯絡資訊，優先級如下：

| 設定項目 | 網站設定 | 頁面內容 | 優先級 |
|---------|---------|---------|-------|
| 客服電話 | ✅ | ✅ (contact/footer) | 網站設定 > 頁面內容 |
| 電子郵件 | ✅ | ✅ (contact/footer) | 網站設定 > 頁面內容 |
| 地址 | ✅ | ✅ (contact/footer) | 網站設定 > 頁面內容 |
| 營業時間 | ✅ | ✅ (contact/footer) | 網站設定 > 頁面內容 |
| 社群連結 | ✅ | ✅ (footer) | 網站設定 > 頁面內容 |
| 版權聲明 | ✅ | ✅ (footer) | 網站設定 > 頁面內容 |
| 品牌描述 | ❌ | ✅ (footer) | 頁面內容 |
| 第二電話/郵箱 | ❌ | ✅ (contact) | 頁面內容 |

建議：使用「網站設定」管理全站通用資訊，「頁面內容」管理特定區塊的額外內容。

### 動態內容管理
前台以下區塊支援從後台 CMS 動態更新：

| 區塊 | 可編輯字段 |
|------|-----------|
| Hero（主視覺） | 標題、副標題、標語1/2、描述、背景圖片、按鈕文字/連結 |
| About（關於我們） | 標題、描述、圖片、引言、統計數據（可視化編輯） |
| Franchise（加盟資訊） | 標題、描述、加盟優勢、加盟步驟、加盟條件、投資金額（均為可視化編輯） |
| Contact（聯絡方式） | 客服專線1/2、電子郵件1/2、地址、服務時間 |
| Footer（頁尾） | 品牌描述、社群連結、聯絡資訊、版權聲明 |

編輯路徑：後台 → 頁面內容管理，保存後前台即時生效（無需重新部署）。

**可視化編輯器**：統計數據、加盟步驟、加盟條件、投資金額等複雜數據現在都有專屬的可視化編輯界面，無需手動編輯 JSON，點擊按鈕即可新增/刪除項目。
